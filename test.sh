...
if [ -f "${CI_PROJECT_DIR}/tests/requirements.yml" ]; then
  ...
  docker exec --tty $container_id env TERM=xterm sed -i s/'\(git@gitlab.com:\)\(\S\+.git\)'/'https:\/\/gitlab-ci-token:${CI_JOB_TOKEN}@gitlab.com\/\2'/g /etc/ansible/roles/role_under_test/tests/requirements.yml
